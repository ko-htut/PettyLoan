...
import com.yixun.pettyloan.utils.NetworkUtil;
...
public class MyWebViewClient extends WebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
        LogUtils.logd(TAG , " shouldOverrideUrlLoading  url : " + url);
        // 优酷视频跳转浏览器播放
    ...
    public void onPageStarted(WebView view, String url, Bitmap favicon) {
        LogUtils.logd(TAG , " onPageStarted  url : " + url + " canGoBack : " + view.canGoBack());
        mActivity.controlClose();
        super.onPageStarted(view, url, favicon);
    }

    @Override
    public void onPageFinished(WebView view, String url) {
        LogUtils.logd(TAG , " onPageFinished  url : " + url + " canGoBack : " + view.canGoBack());
        //从第一个页面回退时，url为about:blank，此时webview.canGoBack()返回true,故不去操作关闭按钮
        ...
        if (!NetworkUtil.isNetWorkAvaliable(mActivity)) {
            mIWebPageView.hindProgressBar();
        }
